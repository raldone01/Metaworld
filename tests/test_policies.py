...
from metaworld.agent import ExpertPolicyMetaworldAgent, run_agent_episode_with_env
...
def test_policies(env_name: str, reward_function_version: str):
    ...
    for _ in range(num_episodes):
        ep_results = run_agent_episode_with_env(
            env=env,
            agent=agent,
            max_episode_steps=max_episode_steps,
        )
    ...
    assert success_rate >= good_policy_success_rate, (
        f"Success rate {success_rate} for env {env_name} below {good_policy_success_rate}"
    )
